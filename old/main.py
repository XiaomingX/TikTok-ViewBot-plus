...
class Captcha:
    ...
    def solve(self) -> None:
        try:
            html           = str(self.client.get('https://zefoy.com', headers=Client.headers()).text).replace('&amp;', '&')
            ...
            captcha_image  = get('https://zefoy.com' + captcha_url, headers=Client.headers(), cookies=self.client.cookies.get_dict()).content
            image          = Image.open(BytesIO(captcha_image))
            image.show()
            
            captcha_answer = input('solve captcha: ')
            
            response = self.client.post('https://zefoy.com', headers=Client.headers({"content-type": "application/x-www-form-urlencoded"}), data={
...
class Zefoy:
    def __init__(self, client: Session) -> None:
        self.client = client
        self.key = Captcha(client).solve()
        self.config = load(open('config.json', 'r'))

    def decode(self, text: str) -> str:
        return b64decode(unquote(text[::-1])).decode()
    
    def send(self, token: str, aweme_id: str) -> None:
        try:
            payload = f"--tekky\r\nContent-Disposition: form-data; name=\"{token}\"\r\n\r\n{aweme_id}\r\n--tekky--\r\n"
            response = self.decode(self.client.post("https://zefoy.com/" + mode, 
    ...
    def search(self, link: str) -> None:
        try:
            payload = f"--tekky\r\nContent-Disposition: form-data; name=\"{self.key}\"\r\n\r\n{link}\r\n--tekky--\r\n"
            response = self.decode(self.client.post("https://zefoy.com/" + mode, 
                data=payload, headers=Client.headers({"content-type": "multipart/form-data; boundary=tekky"})).text.encode())
